...
export const useRedactionEngine = (): UseRedactionEngine => {
  const markForRedaction = useCallback(async (
    proofPack: ProofPack,
    areas: RedactionArea[]
  ): Promise<RedactionPlan> => {
    const response: RedactionResponse<RedactionPlan> = await invoke('mark_for_redaction', {
  ...
  const applyRedactions = useCallback(async (plan: RedactionPlan): Promise<RedactedProofPack> => {
    const response: RedactionResponse<RedactedProofPack> = await invoke('apply_redactions', {
      plan: {
  ...
  const verifyRedactedPack = useCallback(async (
    pack: RedactedProofPack
  ): Promise<PartialVerificationResult> => {
    const response: RedactionResponse<PartialVerificationResult> = await invoke('verify_redacted_pack', {
      redactedPack: {
  ...
  const generateRedactionProof = useCallback(async (
    areas: RedactionArea[]
  ): Promise<RedactionProof[]> => {
    const response: RedactionResponse<RedactionProof[]> = await invoke('generate_redaction_proof', {
      areas: areas.map(area => ({
        ...area,
        coordinates: area.coordinates ? {
          x: area.coordinates.x,
  ...
  const validateRedactionIntegrity = useCallback(async (
    pack: RedactedProofPack
  ): Promise<boolean> => {
    const response: RedactionResponse<boolean> = await invoke('validate_redaction_integrity', {
      redactedPack: {
        originalId: pack.originalId,
        redactedId: pack.redactedId,
        redactionData: pack.redactionData,
    ...
    if (!response.success) {
      throw new Error(response.error || 'Failed to validate redaction integrity');
    }
