        ...
        request: {
          ...
          quality_settings: {
            ...
            screen_resolution_scale:
              config.qualitySettings.screenResolutionScale,
  ...
  describe("Configuration Helpers", () => {
    ...
    it("should validate configuration correctly", () => {
      ...
      expect(invalidErrors).toContain(
        "Screen resolution scale must be between 0.1 and 2.0",
      );
      expect(invalidErrors).toContain(
        "Compression level must be between 1 and 9",
      );
      expect(invalidErrors).toContain(
        "At least one capture type must be enabled",
      );
    });
  });
