...
export class TauriCryptoManager implements CryptoManager {
  ...
  async generateDeviceKey(): Promise<DeviceKey> {
    const response: CryptoResponse<DeviceKey> = await invoke("generate_device_key");
  ...
  async generateKeyPair(): Promise<KeyPair> {
    ...
    return {
      ...keyPair,
      publicKey: new Uint8Array(keyPair.publicKey as any).buffer,
      privateKey: new Uint8Array(keyPair.privateKey as any).buffer,
    };
  }

  async encrypt(data: ArrayBuffer, keyId: string): Promise<EncryptionResult> {
    const dataArray = Array.from(new Uint8Array(data));
    const response: CryptoResponse<[EncryptionResult, string]> = await invoke("encrypt_data", {
      data: dataArray,
      keyId,
    });
  ...
  async sign(data: ArrayBuffer, keyId: string): Promise<CryptoSignature> {
    ...
    const response: CryptoResponse<CryptoSignature> = await invoke("sign_data", {
      data: dataArray,
  ...
  async verify(data: ArrayBuffer, signature: CryptoSignature): Promise<boolean> {
  ...
  async storeKeyInKeychain(keyId: string, keyData: ArrayBuffer): Promise<void> {
    ...
    const response: CryptoResponse<void> = await invoke("store_key_in_keychain", {
      keyId,
  ...
  async retrieveKeyFromKeychain(keyId: string): Promise<ArrayBuffer> {
    const response: CryptoResponse<number[]> = await invoke("retrieve_key_from_keychain", {
      keyId,
    });
  ...
  async deleteKeyFromKeychain(keyId: string): Promise<void> {
    const response: CryptoResponse<void> = await invoke("delete_key_from_keychain", {
      keyId,
    });
